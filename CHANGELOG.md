...
- Fix every instant-closing updater at the launcher layer: new `noctarchy-hold` runs a command, preserves its exit code, and waits for Enter; `noctarchy-terminal` wraps all one-shot commands with it (Extra themes, Channel set, kernel/splash runs, webapp-install, hardware restarts) and the raw `TUI.float` one-shots (5× Maintenance, disk speedtest, capture/QR/transcode, reminders) use it directly. Removed the per-script holds that would now double-prompt ([`4fc6e5b`](https://github.com/deoxizn/noctarchy/commit/4fc6e5b))
- Migrate repo Noctalia config to the v5 schema: v5 renamed widgets (`separator`→`spacer`, `active-window`→`active_window` — old IDs rendered as red blocks), restructured `[bar]`→`[bar.default]` and widget settings into `[widget.*]`, `[notifications]`→`[notification]`, `[lock]`→`[idle.behavior.lock]`, wallpaper mode/interval→`fill_mode`/automation, `[clipboard]`→`[shell]` keys; dropped v5-removed sections. Validates with 0 warnings. `sync.sh` now runs one-shot `noctarchy-noctalia-v5-migrate` on existing v4 configs (backup-first, idempotent) since it otherwise never touches them ([`646dc04`](https://github.com/deoxizn/noctarchy/commit/646dc04), [`6ec543e`](https://github.com/deoxizn/noctarchy/commit/6ec543e))
...
- Fix PrintScreen binds for Niri: `slurp` region-select is dead here, so `Super+Print` now opens Niri's interactive overlay (drag region or click a window, file + clipboard) instead of the broken grim+slurp pipes; `Ctrl+Print` fullscreen-to-clipboard kept; dead `Mod+Print`-to-clipboard removed
...
- Move Cleaner/Snapshots from System into Maintenance as System Cleaner (omacleaner) and Snapshot Manager (snaptui), alphabetized with existing items; System back to 8 entries. `sync.sh` now sets `NoDisplay=true` on their app-launcher entries so they don't duplicate the menu (re-applied every sync since their installers restore the files) ([`fb82d28`](https://github.com/deoxizn/noctarchy/commit/fb82d28), [`6073aec`](https://github.com/deoxizn/noctarchy/commit/6073aec))
...
- Fix double-Enter follow-up: the arch-updater wraps runs in `script -qec` (a pty, for `gum`), which defeated the TTY check — the hold is now also skipped when the parent process is `script` ([`adf0b38`](https://github.com/deoxizn/noctarchy/commit/adf0b38))
