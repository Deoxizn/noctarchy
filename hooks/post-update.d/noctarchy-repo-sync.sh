#!/bin/bash
# noctarchy: post-update hook — keep the repo current after omarchy-update.
# Resolves repo location via state file, XDG default, or baked path.
# Includes a self-healing wrapper: if the repo is missing at the baked path,
# it tries the XDG default and re-runs sync so the new hook gets deployed
# on the same cycle.
...
# Resolve repo directory with auto-migration from legacy paths if needed
resolve_repo() {
  ...
  # 3. Auto-migrate: legacy clone locations → ~/.local/opt/noctarchy
  if [[ ! -d $XDG_DEFAULT ]]; then
    for legacy in "${LEGACY_PATHS[@]}"; do
      if [[ -d $legacy ]]; then
        mkdir -p "$(dirname "$XDG_DEFAULT")"
        if mv "$legacy" "$XDG_DEFAULT" 2>/dev/null; then
          echo "$XDG_DEFAULT"
          return
        fi
      fi
...
exec 9>/tmp/noctarchy-repo-sync.lock || exit 0
